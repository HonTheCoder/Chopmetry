...
const getSpawnDelay = () => {
    ...
    const spawnDelayMin = 300; // lowest allowed
...
// 📌 SHAPE TYPES (MUST BE BEFORE USED)
const shapeTypes = ['cube', 'tetra', 'octa', 'sphere'];

// 📌 SHAPE MODEL FUNCTIONS
function makeTetrahedronModel({ scale = 1 }) {
  const sqrt2over3 = Math.sqrt(2) / 3;
  return {
    vertices: [
      { x: 0, y: 0, z: scale },
      { x: -scale, y: 0, z: -scale / 3 },
      { x: scale, y: 0, z: -scale / 3 },
      { x: 0, y: scale * sqrt2over3 * 2, z: -scale / 3 }
    ],
    polys: [
      { vIndexes: [0, 1, 2] },
      { vIndexes: [0, 2, 3] },
...
function makeOctahedronModel({ scale = 1 }) {
  return {
    vertices: [
      { x: 0, y: 0, z: scale },
      { x: scale, y: 0, z: 0 },
...
// ✅ Low-poly Sphere (Icosahedron-like)
function makeSphereModel({ scale = 1 }) {
    ...
    vertices.forEach(v => {
        ...
        v.x = (v.x / length) * scale;
        v.y = (v.y / length) * scale;
        v.z = (v.z / length) * scale;
    });
    const polys = [
        { vIndexes: [0, 11, 5] }, { vIndexes: [0, 5, 1] }, { vIndexes: [0, 1, 7] }, { vIndexes: [0, 7, 10] }, { vIndexes: [0, 10, 11] },
        { vIndexes: [1, 5, 9] }, { vIndexes: [5, 11, 4] }, { vIndexes: [11, 10, 2] }, { vIndexes: [10, 7, 6] }, { vIndexes: [7, 1, 8] },
        { vIndexes: [3, 9, 4] }, { vIndexes: [3, 4, 2] }, { vIndexes: [3, 2, 6] }, { vIndexes: [3, 6, 8] }, { vIndexes: [3, 8, 9] },
        { vIndexes: [4, 9, 5] }, { vIndexes: [2, 4, 11] }, { vIndexes: [6, 2, 10] }, { vIndexes: [8, 6, 7] }, { vIndexes: [9, 8, 1] }
    ];
    return { vertices, polys };
}

function getShapeModel(shape, scale) {
  switch (shape) {
    case 'tetra':
      return makeTetrahedronModel({ scale: scale * 1.8 }); // ~80% larger
    case 'octa':
      return makeOctahedronModel({ scale: scale * 1.6 }); // ~60% larger
    case 'sphere':
      return makeSphereModel({ scale: scale * 1.35 }); // placeholder same as octa
    case 'cube':
    default:
      return optimizeModel(makeRecursiveCubeModel({
        recursionLevel: 1,
...
function getTargetOfStyle(color, wireframe) {
  ...
  if (!target) {
    shape = pickOne(shapeTypes);
    model = getShapeModel(shape, targetRadius);
    ...
    target.model = model;
    target.vertices = cloneVertices(model.vertices);
    target.shadowVertices = cloneVertices(model.vertices);

    target.polys = model.polys.map(p => ({
      vertices: p.vIndexes.map(i => target.vertices[i]),
      color,
      wireframe,
    ...
    target.shadowPolys = model.polys.map(p => ({
      vertices: p.vIndexes.map(i => target.shadowVertices[i]),
      wireframe,
      normalWorld: { x: 0, y: 0, z: 0 }
    }));
  }
...
/**
 * Adds glow animation when score changes
 */
...
/**
 * Adds glow animation when cube count changes
 */
function updateCubeCountDisplay(cubes) {
  const cubeEl = document.querySelector('.cube-count-lbl');
  cubeEl.textContent = `CUBES SMASHED: ${cubes}`;

  cubeEl.classList.remove('glow');
  void cubeEl.offsetWidth;
  cubeEl.classList.add('glow');
}

/**
 * Controls visibility and content of HUD based on game mode
 */
function renderScoreHud() {
  if (isCasualGame()) {
    ...
    updateScoreDisplay(state.game.score); // Use animation-enhanced function
    scoreNode.style.display = 'block';
    cubeCountNode.style.opacity = 0.65;
  }

  updateCubeCountDisplay(state.game.cubeCount);
}
...
function tick(width, height, simTime, simSpeed, lag) {
    ...
    const maxForceMultiplier = 2.5; // tweak this value as needed
    const cappedForceMultiplier = Math.min(forceMultiplier, maxForceMultiplier);
    pointerDelta.x = 0;
    pointerDelta.y = 0;
    ...
    for (let i = targets.length - 1; i >= 0; i--) {
        ...
        if (target.y > centerY + targetHitRadius * 2) {
            ...
            // ✅ Only penalize if NOT a slow-mo power-up (wireframe)
            if (!target.wireframe && isInGame()) {
                if (isCasualGame()) {
                    incrementScore(-25);
                } else {
                    endGame();
        ...
        for (let ii = 1; ii <= hitTestCount; ii++) {
            ...
            const hitRadiusMultiplier = isMobile ? 1.3 : 1.0;

            if (distance <= targetHitRadius * hitRadiusMultiplier) {
                if (!target.hit) {
                    const sparkSpeed = 7 + pointerSpeedScaled * 0.125;

                    target.hit = true;
                    target.hitCooldown = 100;

                    target.health--; // ✅ Always subtract health regardless of slice speed

                    target.xD += pointerDeltaScaled.x * hitDampening * (cappedForceMultiplier / forceMultiplier);
                    ...
                    const isStrong = pointerSpeedScaled > minPointerSpeed;
                    const slowmoPenalty = (slowmoRemaining > 0) ? 0.5 : 1;
                    const baseScore = isStrong ? 10 : 5;
...
function animateParticles() {
    ...
    particleCtx.fillStyle = "rgba(0,255,255,0.6)";
    particles.forEach(p => {
        p.x += p.dx;
        p.y += p.dy;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toggleThemeBtn) {
        toggleThemeBtn.addEventListener('click', () => {
            const isNeon = body.classList.contains('theme-neon');

            if (isNeon) {
                body.classList.remove('theme-neon');
                body.classList.add('theme-light');
                toggleThemeBtn.textContent = '🌙 THEME: LIGHT';
            } else {
                body.classList.remove('theme-light');
                body.classList.add('theme-neon');
                toggleThemeBtn.textContent = '🎨 THEME: NEON';
            }
        });
    }
});
